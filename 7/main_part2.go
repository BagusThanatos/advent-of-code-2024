...
func tryCalculate (currentTotal , total int64, currentLine, index int) bool {
  ...
  first := strconv.FormatInt(currentTotal, 10)
  second := strconv.FormatInt(lines[currentLine][index], 10)
  combined,_ := strconv.ParseInt( first + second , 10, 64)
  return tryCalculate(combined, total, currentLine, index+1) ||
  tryCalculate(currentTotal * lines[currentLine][index], total, currentLine, index+1) ||
  tryCalculate(currentTotal + lines[currentLine][index], total, currentLine, index+1)
}
